...
class HRI_communication():
    ...
    def _run_avatar(self):
        ...
        while count<settings['n_readings']:
            ...
            # check if unity flag
            unity_flag = self._get_unity_flag()
...
def _control_routine(input_data_num, mapp, control_history_raw_num, control_history_num, count):
    ...
    if  settings['control_from_dummy_data']: ###TOFIX
        pass
        # input_data_num = self.dummy_data[self.count:self.count+1]
    # else get data from input device
    else:
    ...
    controls = y_score_scaled.tolist() + [0, 0, 0] # comment for HW ### TODO : make option
    # store in history input data array ### TODO : do outside of function
    control_history_num[count] = y_score_scaled
...
def _get_unity_flag():
    ...
    flag = udp.udp_read(udp.sockets['read_unity_flag'])
...
def _import_mapping(is_struct = False):
    ...
    def extract_parameters(normalization_values, used_body_parts, outputs, outputs_no_pll):
        ...
        if settings['input_device'] == 'motive' or settings['input_device'] == 'imus':
            ...
            feats = [x + '_' + str(y) for y in used_body_parts for x in HRI_mapping.select_motive_features()]
    ...
    if is_struct:
        ...
        # load mapping
        mapp =  HRI.load_obj(os.path.join(settings['subject_folder'], settings['control_style']))
        # puts data in dict
...
def _read_unity_data(unity_num, count):
...
def _run_acquisition(count = 0):
    ...
    while not stop:
        ...
        if count>=settings['n_readings'] or (count>0 and elapsed>idle):
        ...
        if unity_flag=='a':      
            ...
            # read unity data
            unity_temp, unity_num = _read_unity_data(unity_num, count)
            # increment counter
            count += 1
...
def _run_control(mapp):
    ...
    while not stop:
        ...
        if not settings['control_from_dummy_data']:
            ...
            unity_flag = 'c' if settings['simulate_flag'] else _get_unity_flag()

        # if flag=='c' : read unity and input data, send commands to unity
        if unity_flag=='c':

            # acquire unity data
            _, unity_num = _read_unity_data(unity_num, count)
...
def _skeleton_preprocessing(skel):
    ...
    skel_first = np.copy(skel.first_data.values)
    ...
    # used for debug
    _DEBUG['skel'] = np.copy(skel.values)
    _DEBUG['skel_bodyparts'] = skel_bodyparts
    _DEBUG['skel_rel'] = skel_rel
    _DEBUG['skel_first'] = skel_first
...
def acquire_store_init_pose():
    ...
    logging.info('acquired initial pose')

    HRI.save_obj(data_proc, os.path.join(os.path.dirname(__file__),'..','init_pose'))
...
def get_init_pose():
    ...
    return HRI.load_obj(os.path.join(os.path.dirname(__file__),'..','init_pose'))
...
def _skeleton_preprocessing_II(skel, mapp):
    ...
    # normalize
    skel.normalize(mapp['parameters']['norm_av'], mapp['parameters']['norm_std'], feats = settings['features_used'])
    ...
    if settings['dim_reduction']:
        ...
        if settings['dim_reduction_type'] == 'bones':
            ...
            skel_reg = skel.extract_features(mapp['features'])
            skel_tofit = skel_reg.reshape(1, -1)
...
def _store_control_to_file(control_history_raw_num, control_history_num, unity_num):
    ...
    unity_history = np.vstack([settings['headers']['unity_calib'], np.array(unity_num)])
...
def run(mode, old_mapping = False):
    ...
    if mode == 'control':
        ...
        if settings['control_from_dummy_data']:
            # import dummy data, if specified
            _import_dummy()
        else:
            # else import mapping data for control
