...
folder = '/Volumes/GoogleDrive/My Drive/Matteo/EPFL/LIS/PhD/Natural_Mapping/DATA/Unity_data/experiments_dirty/motive_maxmin'

info = 'interface_motive_method_maxmin'

useful_file_size_min = 900

baseline_folder = os.path.join(folder, 'baseline')
garbage_folder = os.path.join(folder, 'garbage')
maneuvers_folder = os.path.join(folder, 'maneuvers')
useless_folder = os.path.join(folder, 'useless')

utils.create_dir_safe(baseline_folder)
utils.create_dir_safe(garbage_folder)
utils.create_dir_safe(maneuvers_folder)
utils.create_dir_safe(useless_folder)
...
for file in files:
    ...
    new_name = new_name.replace('s4_evaluation_manoeuvreList', 'file_maneuvers')
    new_name = new_name.replace('s4_evaluation_TimeAngleRatePosRot', 'file_garbage')
    new_name = new_name.replace('s4_evaluation_waypointDistTime', 'file_performance')
